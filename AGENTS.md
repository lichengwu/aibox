...
aibox check [module]                      # preflight: environment, or one module's readiness
aibox list / list-available / ports / dashboard [module]
...
12. **Residue map for `aibox-purge`**: extend `scripts/purge.sh` (`residue_*` functions) with the module's leftovers (volumes / containers / `/etc/<name>` / units / dispatched binaries / npm packages) — the standalone cleaner must be able to remove residue even AFTER aibox itself is uninstalled (it survives `self uninstall` on purpose; `aibox clean` forwards to it). Validator WARNs when the entry is missing; spec: `docs/module-spec.md` §Residue cleanup.
