...
- **Bootstrap / self-update**: `install.sh` and `aibox update self` run `curl|bash` from `raw.githubusercontent.com`. Since v0.4.0 the bootstrap supports **checksum verification** (`AIBOX_SHA256` to pin, or `AIBOX_VERIFY=1` to check the release `SHA256SUMS` sidecar, graceful if absent). The release workflow attaches a `SHA256SUMS` asset. **Boundary:** the check verifies the payload `bin/aibox`, **not `install.sh` itself** — a `curl|bash` MITM can serve a malicious `install.sh` that skips the check. This is inherent to `curl|bash`; for full assurance pin `AIBOX_SHA256` from a trusted channel or use `AIBOX_RAW=file://`. Also note: with `AIBOX_VERIFY=1`, `aibox update self` fetches `bin/aibox` from raw `main` but the sidecar from `releases/latest`, so a mismatch can occur when `main` is ahead of the latest release — the check is most reliable right after a release.
...
- **Destructive ops**: `aibox uninstall self` is fail-safe — the default removes ONLY the manager (module services and data are KEPT; `apps/` is preserved so surviving deployments stay manageable); `--purge` cascades a full teardown (every installed module's hook with `AIBOX_PURGE_DATA=1`, then `apps/`, then the manager). Confirm gate: TTY asks; non-interactive requires `--yes`. After-the-fact residue is handled by `aibox purge` — dry-run by default, and `--apply` refuses RUNNING containers/processes without `--stop`.
...
- Pin the version: `AIBOX_SHA256=<expected> aibox update self`.
