...
- **Single instance:** fixed `container_name: aibox-gitlab-ce` + project-scoped
  named volumes (project = deploy-root basename `gitlab-ce`). Multi-instance
...
- **install ≠ start:** booting GitLab is heavy (3-5 min, 4 GB RAM); the install
  hook only places files, `aibox gitlab-ce start` boots and waits (poll
...
- First boot: container health stays `starting` for minutes (start_period 300s
  in the compose healthcheck); `aibox gitlab-ce status` surfaces the health
...
```bash
scripts/validate-module.sh gitlab-ce     # conformance (same rules as CI)
bats tests/*.bats                        # fast suite
# live (needs docker + ~4GB RAM + ~4GB disk):
aibox install gitlab-ce && aibox gitlab-ce start && aibox gitlab-ce credentials
```
