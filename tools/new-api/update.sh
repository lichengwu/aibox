#!/usr/bin/env bash
# new-api module — update hook. Refreshes the deployed compose from the
# freshly-downloaded module cache; never clobbers the deploy .env (secrets /
# user config / the floated image tag live there). aibox passes
...
log "kept existing ${ROOT}/.env (NEW_API_IMAGE there overrides the compose floor)"

# Apply: recreate containers so the new compose/env take effect.
case "${1:-}" in
--restart)
  log "applying (--restart): recreating container…"
